...
uses
  System.UITypes, Androidapi.JNI.App, Androidapi.JNI.GraphicsContentViewText,
  FMX.Graphics;
...
uses
  System.Classes, FMX.Helpers.Android, FMX.Platform.Android, FMX.Surfaces,
  FMX.Forms, FMX.Styles, FMX.Controls,
...
function AlphaColorToJColor(const AColor: TAlphaColor): Integer;
begin
  Result := TJColor.JavaClass.argb(TAlphaColorRec(AColor).A,
  ...
  begin
    if Form.StyleBook <> nil then
      LStyleDescriptor := TStyleManager.FindStyleDescriptor
        (Form.StyleBook.Style)
    else
      LStyleDescriptor := TStyleManager.FindStyleDescriptor
        (TStyleManager.ActiveStyleForScene(Form as IScene));
